...
function App() {
      ...
      const passDataIndex = selecteKPIV.map((kpiv, kpivIndex) => {
        ...
        json_data_cba.forEach((item, index) => {
          ...
          if (
            ...
            seriesData.push({
              x: index + 1,
              y: selecteKPIVFromJson[kpivIndex][index],
            });
          } else {
            seriesData.push({
              x: index + 1,
  ...
  const Submit_KPOV = async () => {
    ...
    try {
      ...
      if (result_data_bin) {
        const parsedData = JSON.parse(result_data_bin);
        setTableData(parsedData);
      } else {
        console.error("No data or invalid response from the API");
      }

      console.log("result_data_bin", result_data_bin);
      setShowTable(true);
          ...
          {countAfterSMOTE && (
            <div className="row">
              <div className="col-2 mt-3">
                <div className="form-group">
                  <label htmlFor="maxKPOV">support</label>
                </div>{" "}
                0.
                <input
                  type="text"
                  onChange={(e) => setSupportValue(e.target.value)} // ใช้ setSupportValue เพื่ออัปเดตค่า supportValue
                  value={supportValue}
                  placeholder="Enter a value"
                />
                <div className="form-group">
                  <label htmlFor="maxKPOV">Confidence</label>
                </div>{" "}
                0.
                <input
                  type="text"
                  onChange={(e) => setConfidenceValue(e.target.value)} // ใช้ setConfidenceValue เพื่ออัปเดตค่า confidenceValue
