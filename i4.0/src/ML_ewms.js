...
import { PythonAPI_make_chartML_ewms } from "./api";
import { pairplot } from "./api"; // Adjust the path accordingly
import { summary_describe_ewms } from "./api";
import { Parameter } from "./api";
import Select from "react-select";
import "bootstrap/dist/css/bootstrap.min.css"; // นำเข้า CSS ของ Bootstrap
import OverlayTrigger from "react-bootstrap/OverlayTrigger";
import Tooltip from "react-bootstrap/Tooltip";
import { getModelOptions_ewms } from "./api";
import { getlineOptions_ewms } from "./api";
import { getParameterOptions_ewms } from "./api";
import { BinKPOV_Auto_ewms } from "./api";
import { BIN_KPOV_ewms } from "./api";
import { BIN_KPIV_ewms } from "./api";
...
function App() {
  ...
  const handleRadioChange_KPOV = async (event) => {
    ...
    if (selectedValue === "yes") {
      setIsReadOnly(true);
      try {
        const response = await BinKPOV_Auto_ewms(model, selecteKPOV);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const summaryData = await summary_describe_ewms(
        model,
        line,
        start,
        end,
      ...
      const pythonApiUrl = await PythonAPI_make_chartML_ewms();
  ...
  const SubmitButton_KPIV = async () => {
    ...
    try {
      ...
      const result_KPIV = await BIN_KPIV_ewms(
        model,
        selecteKPOV,
        selecteKPIV,
        minKPOV,
  ...
  const Submit_KPOV = async () => {
    setIsLoading(true);
    try {
      const result = await BIN_KPOV_ewms(selecteKPOV, minKPOV, maxKPOV);
  ...
  const fetchModelOptions = async () => {
    try {
      const options = await getModelOptions_ewms(); // เรียกใช้งาน API หรือฟังก์ชันเพื่อรับข้อมูล Model
  ...
  const fetchLineOptions = async (model) => {
    ...
    try {
      const options = await getlineOptions_ewms(model); // Use 'model' in the API request
  ...
  const fetchParameterOptions = async () => {
    try {
      const options = await getParameterOptions_ewms(); // Replace with your API call
