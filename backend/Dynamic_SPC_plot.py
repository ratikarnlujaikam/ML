...
# Set labels and title
    ...
    model = data['model'][0]
    x = datetime.now()
    Date_new = (x.strftime("%X")).replace(':', '-')
    plt.title(f'SPC {model} MC{Machine}')
   
# Show legend
    plt.legend()
# Show the plot
    plt.savefig(f'../TrainingNodeJS/chart/{model}_MC_{Machine}.png')
    plt.close()
...
def input(model, Line, process, start):
...
def Senfile_input(model,Line,process,start):
    model = 'LONGSP'
    Line = '3-6'
    process = 'Projection1'
    start = '2024-01-08'
    ...
    return send_file(f'../TrainingNodeJS/chart/{model}_MC_{arrayMC[0]}.png', mimetype='image/png')
