    ...
    try:
        ...
        cursor.execute("""
          with set1 as (SELECT DISTINCT [Part] + '>' + [parameters].[Parameter] AS parameters
            ...
            AND [Master_matchings].Model='{model}'
...
def get_model():
    try:
        ...
        # ดึงข้อมูลจาก SQL
        cursor.execute(
            "SELECT distinct[Model] as model FROM [Component_Master].[dbo].[Master_matchings]")
        rows = cursor.fetchall()
...
def BIN_KPOV( selecteKPOV, minKPOV, maxKPOV):
    ...
    try:
        ...
        error_message = str(e)
        print("An error occurred:", error_message,count_before_smote)
