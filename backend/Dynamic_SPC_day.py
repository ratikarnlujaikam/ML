...
def process_input(process, startDate,model, line):
    process_1 = 'Projection1'
    MC_NAME = process
    
    data = fetch_data(model, line, startDate, process_1, MC_NAME)
...
def get_process():
    try:
        ...
        # ดึงข้อมูลจาก SQL
        cursor.execute(
            " select  'Dimension_WR' as process union select 'Dynamic_Parallelism_Tester'")
        rows = cursor.fetchall()
